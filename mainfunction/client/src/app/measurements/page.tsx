...
export default function MeasurementsPage() {
  ...
  const getThemeColor = () => {
    switch (selectedType) {
      case "glucose":
        return "#5EEAD4"; // Soft Teal
      case "bp":
        return "#93C5FD"; // Soft Blue
      case "weight":
        return "#C4B5FD"; // Soft Purple
      default:
        return "#7A8E6B";
    }
  };
  ...
  return (
    <ProtectedRoute>
      <DashboardLayout>
        ...
        {/* Chart Section */}
        <div className="bg-white border border-gray-100 rounded-xl p-8 mb-8 shadow-sm">
          <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
            {[
              ...
              <button
                ...
                className={`flex items-center space-x-3 px-6 py-3  font-bold transition-all whitespace-nowrap ${
                  selectedType === type.id
                    ? "bg-white border-b-2 border-gray-500 text-gray-900"
                    : "hover:bg-gray-50 text-gray-500"
                }`}
              >
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center ${type.bg} ${type.color}`}
                >
                  <type.icon />
                </div>
          ...
          {measurements.length === 0 ? (
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
              {measurements.map((measurement) => (
                <div
                  key={measurement._id}
                  className="group flex items-center justify-between p-5 rounded-xl border border-gray-100 hover:border-[#7A8E6B]/30 hover:shadow-sm transition-all duration-300"
                >
                        ...
                        <div key={idx} className="flex items-center space-x-2">
                          ...
                          <span className="text-gray-900 font-bold">
                            {reading.type === "bloodPressure"
                              ...
                              : reading.value}
                            <span className="text-gray-400 text-sm ml-1 lowercase">
                              {reading.unit}
                            </span>
                          </span>
                        </div>
                  ...
                  <div className="text-sm text-gray-400">
                    {new Date(measurement.date).toLocaleTimeString(undefined, {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </div>
